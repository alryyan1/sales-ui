...
export interface StockAdjustment {
  ...
  warehouse_id?: number | null; // Add warehouse_id
  user_id: number | null;
...
const stockAdjustmentService = {
  ...
  ): Promise<{ adjustment: StockAdjustment; product: Product }> => {
    // Return adjustment log and updated product
    try {
      // Backend endpoint likely returns the log and updated product state
      const response = await apiClient.post<{
        adjustment: StockAdjustment;
        product: Product;
      }>("/stock-adjustments", adjustmentData);
      ...
      if (axios.isAxiosError(error) && error.response?.status === 422) {
        console.warn(
          "Validation error during stock adjustment:",
          error.response.data
        );
      }
