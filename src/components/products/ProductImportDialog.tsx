...
const ProductImportDialog: React.FC<ProductImportDialogProps> = ({
  ...
  const productFields = [
    { key: 'name', label: t('products:productName'), required: true },
    { key: 'sku', label: t('products:form.skuLabel'), required: false },
    { key: 'stock_quantity', label: t('products:stockQuantity'), required: false },
  ];
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!selectedFile.name.match(/\.(xlsx|xls)$/)) {
      setError(t('products:import.invalidFileTypeUnsupported'));
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('products:import.uploadFileFailedRetry'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('products:import.previewFailedCheckMapping'));
    } finally {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        if (err.message.includes('timeout') || err.message.includes('Network Error')) {
          errorMessage = t('products:import.timeoutError');
        } else if (err.message.includes('413')) {
          errorMessage = t('products:import.fileTooLarge');
        } else {
    ...
    <Box>
      ...
      >
        ...
        <Typography variant="h6" sx={{ mb: 1 }}>
          {t('products:import.chooseImportFile')}
        </Typography>
  ...
  const renderStep2 = () => (
    <Box>
      <Typography variant="body1" sx={{ mb: 2 }}>
        {t('products:import.step2Body')}
      </Typography>

      {autoMapped && (
        <Alert severity="info" sx={{ mb: 2 }}>
          <Typography variant="body2">
            {t('products:import.autoMappedMessage')}
          </Typography>
        </Alert>
      )}
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
        ...
        <Button
          ...
          sx={{ ml: 'auto' }}
        >
          {t('products:import.autoMapColumns')}
        </Button>
        <Button
          variant="outlined"
          size="small"
        ...
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell>{t('products:import.excelColumnHeader')}</TableCell>
                ...
                <TableCell>
                  {field.required ? (
                    <Typography color="error" variant="caption">
                      {t('products:import.required')}
                    </Typography>
                  ) : (
                    <Typography color="text.secondary" variant="caption">
                      {t('products:import.optionalField')}
                    </Typography>
                  )}
                </TableCell>
              </TableRow>
    ...
    <Box>
      ...
      {previewLoading && (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
          ...
          <Typography variant="h6" sx={{ mt: 2, mb: 1 }}>
            {t('products:import.generatingPreview')}
          </Typography>
        </Box>
      )}
      ...
      {!previewLoading && previewData.length > 0 && (
        <Box>
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>{t('products:form.skuLabel')}</TableCell>
                  <TableCell>{t('products:stockQuantity')}</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            {t('products:import.previewNote', { count: previewData.length })}
          </Typography>
        </Box>
      )}
      ...
      {!previewLoading && previewData.length === 0 && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          {t('products:import.noPreviewData')}
        </Alert>
      )}
    </Box>
  );
    ...
    <Box>
      ...
      {loading && (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>
            {t('products:import.processingDescription')}
          </Typography>
        </Box>
      )}
      ...
      {importResult && !loading && (
        <Alert severity={importResult.errors === 0 ? 'success' : 'warning'} sx={{ mb: 2 }}>
          <Typography variant="body2">
            {t('products:import.importResultDetailed', { imported: importResult.imported, errors: importResult.errors })}
          </Typography>
        </Alert>
      )}
      ...
      <DialogActions>
        ...
        {activeStep === 3 && !importResult && (
          <Button onClick={handleImport} variant="contained" disabled={loading}>
            {loading ? t('products:import.importingEllipsis') : t('products:import.executeImport')}
          </Button>
        )}
      </DialogActions>
    </Dialog>
