...
// Services and Types
import productService, { Product, PaginatedResponse } from '../services/productService'; // Use product service
...
const ProductsPage: React.FC = () => {
    ...
    const [productToDeleteId, setProductToDeleteId] = useState<number | null>(null); // Use product ID
    ...
    const fetchProducts = useCallback(async (page: number, search: string) => {
        ...
        try {
            ...
            const data = await productService.getProducts(page, search /*, sorting? */);
            setProductsResponse(data);
        } catch (err) { setError(productService.getErrorMessage(err)); }
    ...
    // --- Modal Handlers ---
    const openModal = (product: Product | null = null) => { setEditingProduct(product); setIsModalOpen(true); };
    const closeModal = () => { setIsModalOpen(false); };
    const handleSaveSuccess = () => {
        const messageKey = editingProduct ? 'products:saveSuccess' : 'products:saveSuccess'; // Add keys
        closeModal(); showSnackbar(t(messageKey), 'success');
    ...
    // --- Deletion Handlers ---
    const openConfirmDialog = (id: number) => { setProductToDeleteId(id); setIsConfirmOpen(true); };
    const closeConfirmDialog = () => { if (!isDeleting) { setIsConfirmOpen(false); setTimeout(() => setProductToDeleteId(null), 300); } };
    const handleDeleteConfirm = async () => {
        if (!productToDeleteId) return; setIsDeleting(true);
        try {
            ...
            closeConfirmDialog();
            // Smart refetch
            if (productsResponse && productsResponse.data.length === 1 && currentPage > 1) { setCurrentPage(prev => prev - 1); }
            else { fetchProducts(currentPage, debouncedSearchTerm); }
        } catch (err) { showSnackbar(productService.getErrorMessage(err), 'error'); closeConfirmDialog(); }
    ...
    return (
        <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }} className="dark:bg-gray-900 min-h-screen">
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: 'center', mb: 3, gap: 2 }}>
             ...
             {/* Search Input */}
             <Box sx={{ mb: 3 }}>
                 <TextField fullWidth variant="outlined" size="small" placeholder={t('products:searchPlaceholder') || "Search Products..."} value={searchTerm} onChange={handleSearchChange}
            ...
            {!isLoading && !error && productsResponse && (
                 <Box sx={{mt: 2}}>
                     <ProductsTable products={productsResponse.data} onEdit={openModal} onDelete={openConfirmDialog} isLoading={isDeleting} />
                     {/* Pagination */}
                      {productsResponse.last_page > 1 && ( <Box sx={{ display: 'flex', justifyContent: 'center', p: 2, mt: 3 }}> <Pagination count={productsResponse.last_page} page={currentPage} onChange={handlePageChange} color="primary" shape="rounded" showFirstButton showLastButton disabled={isLoading || isDeleting} /> </Box> )}
                       {/* No Products Message */}
                      {productsResponse.data.length === 0 && ( <Typography sx={{ textAlign: 'center', py: 5 }} className="text-gray-500 dark:text-gray-400">{t('products:noProducts')}</Typography> )}
                 </Box>
            )}

            {/* Modals and Snackbar */}
            <ProductFormModal isOpen={isModalOpen} onClose={closeModal} productToEdit={editingProduct} onSaveSuccess={handleSaveSuccess} />
            <ConfirmationDialog open={isConfirmOpen} onClose={closeConfirmDialog} onConfirm={handleDeleteConfirm} title={t('common:confirmDeleteTitle')} message={t('products:deleteConfirm')} confirmText={t('common:delete')} cancelText={t('common:cancel')} isLoading={isDeleting} /> {/* Add deleteConfirm key */}
