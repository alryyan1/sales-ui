    ...
    const fetchPdf = async () => {
      ...
      try {
        ...
        if (err.name !== "CanceledError") {
          setError(t("products:barcodeLabel.generateFailed"));
        }
      } finally {
        setLoading(false);
      }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ pb: 1 }}>
        <Typography variant="subtitle1" fontWeight={600}>
          {t("products:barcodeLabel.printBarcodeTitle", { name: product.name })}
        </Typography>

        <Stack direction="row" gap={2} mt={1.5} flexWrap="wrap">
          <TextField
            label={t("products:barcodeLabel.widthMm")}
            type="number"
            size="small"
            value={labelWidthMm}
          ...
          />
          <TextField
            label={t("products:barcodeLabel.heightMm")}
            type="number"
            size="small"
            value={labelHeightMm}
          ...
          />
          <TextField
            label={t("products:barcodeLabel.copies")}
            type="number"
            size="small"
            value={copies}
