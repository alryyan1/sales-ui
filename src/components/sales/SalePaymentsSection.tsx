...
export const SalePaymentsSection: React.FC<SalePaymentsSectionProps> = ({
    ...
    const totalPaid = watchedPayments?.reduce((sum: number, p: any) => sum + (Number(p?.amount) || 0), 0) ?? 0;
    ...
    return (
        <div className="space-y-4">
             <div className="flex justify-between items-center mb-4">
                 <h3 className="text-lg font-medium text-gray-800 dark:text-gray-100">{t('sales:paymentsSectionTitle')}</h3>
                 <Button type="button" variant="outline" onClick={addPaymentLine} disabled={isSubmitting || totalPaid >= grandTotal} size="sm" className="text-xs">
                    <PlusCircle className="me-2 h-4 w-4" /> {t('sales:addPaymentMethod')} {/* Add key */}
                </Button>
            </div>

            {/* Display root error for payments array (e.g., from refine if total paid > total sale) */}
            {errors.payments && !Array.isArray(errors.payments) && (errors.payments as any).message && (
                 <Alert variant="destructive" className="mb-4">
                     <AlertCircle className="h-4 w-4" />
                     <AlertDescription>{t((errors.payments as any).message)}</AlertDescription>
                 </Alert>
              )}
            ...
            {fields.map((item, index) => (
             ...
             {fields.length === 0 && (
                 <p className="text-sm text-muted-foreground">{t('sales:noPaymentsAdded')}</p> // Add key
             )}
        </div>
    );
};