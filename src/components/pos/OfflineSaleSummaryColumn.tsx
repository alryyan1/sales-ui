...
export const OfflineSaleSummaryColumn: React.FC<OfflineSaleSummaryColumnProps> = ({
  ...
  const paidAmount = preciseSum(payments.map(p => Number(p.amount)), 2);
      ...
      <Card sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: 'blue.50', p: 2, borderRadius: 1 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', textAlign: 'center' }}>
                  ...
                  <Typography variant="body2" fontWeight="semibold">
                    {dayjs(currentSale.offline_created_at || Date.now()).format('HH:mm A')}
                  </Typography>
                </Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, flex: 1 }}>
                ...
                {/* Total Amount */}
                <Box sx={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.5rem', fontWeight: 'bold', borderTop: 1, borderColor: 'divider', pt: 1, mt: 'auto' }}>
              ...
              >
                ...
                {currentSale.status === 'completed' ? 'تم اكتمال البيع' : (paidAmount > 0 ? 'إدارة الدفع / إكمال' : 'الدفع')}
              </Button>
        </CardContent>
      </Card>
