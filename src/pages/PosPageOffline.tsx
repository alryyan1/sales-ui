...
export const PosPageOffline = () => {
    ...
    const [currentSale, setCurrentSale] = useState<OfflineSale>(offlineSaleService.createDraftSale());
    const [inputValue, setInputValue] = useState('');
    const inputRef = useRef<HTMLInputElement>(null);

    // Dialog State
    // Removed legacy payment dialog state
    ...
    const updateBatch = (productId: number, batchId: number | null, _batchNumber: string | null, _expiryDate: string | null, unitPrice: number) => {
        updateCurrentSale(prev => {
            const newItems = prev.items.map(i =>
                i.product_id === productId
    ...
    const loadPendingSales = async () => {
        ...
        // Sort by date desc
        setPendingSales(sales.sort((a, b) => b.offline_created_at - a.offline_created_at));
    };
    ...
    const cartItems: CartItem[] = useMemo(() => {
        return currentSale.items.map(item => ({
            ...
            selectedBatchNumber: (item.product as Product).available_batches?.find(b => b.id === item.purchase_item_id)?.batch_number,
            selectedBatchExpiryDate: (item.product as Product).available_batches?.find(b => b.id === item.purchase_item_id)?.expiry_date,
        }));
    }, [currentSale.items]);
    ...
    return (
        <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#f4f6f8' }}>
            ...
            >
                <Typography variant="h5" fontWeight="900" sx={{ background: 'linear-gradient(45deg, #2563eb, #7c3aed)', backgroundClip: 'text', color: 'transparent', mr: 4 }}>
                    NEXT POS
                </Typography>
                ...
                <Autocomplete
                    ...
                    renderInput={(params) => (
                        <TextField
                            ...
                            size="small"
                            sx={{ width: 400, bgcolor: 'grey.50', '& fieldset': { borderRadius: 3 } }}
                            onKeyDown={(e) => {
                                if (e.key === 'Enter' && inputValue) {
                                    // Try exact SKU match first for scanner
                                    const exactMatch = products.find(p => p.sku === inputValue || p.sku === inputValue.trim());
                                    if (exactMatch) {
                        ...
                        <li {...props} key={option.id}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                ...
                                <Typography color="primary.main" fontWeight="bold">
                                    {(() => {
                                        let price = Number(option.last_sale_price_per_sellable_unit || 0);
                                        if (price === 0 && option.available_batches && option.available_batches.length > 0) {
                                            price = Number(option.available_batches[0].sale_price || 0);
                                        }
                    ...
                    filterOptions={(options, state) => {
                        ...
                        // Sort exact SKU matches to top for better scanner experience
                ...
                <Stack direction="row" spacing={2} alignItems="center">
                    <Chip
                        icon={isSyncing ? <CircularProgress size={16} /> : (isOnline ? <Wifi /> : <WifiOff />)}
                        label={isSyncing ? "Syncing..." : (isOnline ? "Online" : "Offline Mode")}
            ...
            <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                ...
                <Box sx={{ width: 90, flexShrink: 0, bgcolor: 'white', borderRight: '1px solid', borderColor: 'divider' }}>
                   <PendingSalesColumn 
                       sales={pendingSales} 
                ...
                <Box sx={{ flex: 1, p: 3, display: 'flex', flexDirection: 'column' }}>
                    <CurrentSaleItemsColumn
                        ...
                        onUpdateBatch={async (id, batchId, num, expiry, price) => updateBatch(id, batchId, num, expiry, price)}
                    />
                </Box>

                {/* RIGHT: SUMMARY PANEL */}
                {/* RIGHT: SUMMARY PANEL */}
                <Box sx={{ width: 400, borderLeft: '1px solid', borderColor: 'divider', zIndex: 10, bgcolor: 'white' }}>
