...
const PAYMENT_METHODS: { value: PaymentMethod; label: string; icon: React.ReactNode; color: string }[] = [
    ...
    { value: 'bank_transfer', label: 'تحويل بنكي', icon: <BankIcon />, color: '#6366F1' }, // Indigo
];
...
export const OfflinePaymentDialog: React.FC<OfflinePaymentDialogProps> = ({
  ...
  const handleAddPayment = async () => {
    ...
    try {
        ...
        const updatedSale = {
            ...
            paid_amount: preciseSum(updatedPayments.map(p => Number(p.amount)), 2)
        };
  ...
  const getMethodDetails = (methodValue: string) => {
      return PAYMENT_METHODS.find(m => m.value === methodValue) || { label: methodValue, icon: <CreditCardIcon />, color: '#64748B' }; 
  }
  ...
  const renderQuickAmounts = () => {
      ...
      const uniqueSuggestions = Array.from(new Set(suggestions)).filter(v => v > 0);
  ...
  return (
    ...
    >
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 1 }}>
            <Typography variant="h6" fontWeight="bold">إدارة الدفع</Typography>
        ...
        <DialogContent sx={{ p: { xs: 2, md: 4 } }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 4 }}>
                    ...
                    <Box sx={{ flex: { md: 7 }, minWidth: 0 }}>
                        {/* Summary Header for Entry */}
                         <Box sx={{ mb: 3, p: 2, bgcolor: 'primary.50', borderRadius: 2, border: '1px solid', borderColor: 'primary.100' }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center">
                                <Typography variant="subtitle1" color="primary.800">المبلغ المستحق</Typography>
                                <Typography variant="h5" fontWeight="bold" color="primary.main">
                                    {formatNumber(remainingDue)} <span style={{ fontSize: '0.8rem' }}>SDG</span>
                                </Typography>
                            </Stack>
                         </Box>
                            ...
                            {PAYMENT_METHODS.map((method) => (
                                <Paper
                                    ...
                                    sx={{
                                        ...
                                        bgcolor: paymentMethod === method.value ? alpha(method.color, 0.05) : 'transparent',
                                        borderRadius: 2,
                                        cursor: 'pointer',
                                        display: 'flex',
                                        alignItems: 'center',
                                ...
                                >
                                    <Box sx={{ color: method.color, display: 'flex' }}>
                                        {React.cloneElement(method.icon as React.ReactElement, { fontSize: 'small' })}
                                    </Box>
                                    <Typography variant="body2" fontWeight={paymentMethod === method.value ? 'bold' : 'medium'}>
                                        {method.label}
                                    </Typography>
                                </Paper>
                            ))}
                        ...
                        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                            <TextField 
                                ...
                                helperText={error}
                                InputProps={{
                                    startAdornment: <InputAdornment position="start">SDG</InputAdornment>,
                            ...
                            >
                                {addingPayment ? <CircularProgress size={24} color="inherit" /> : 'إضافة'}
                            </Button>
                        </Box>
                        {renderQuickAmounts()}
                ...
                <Box sx={{ 
                    ...
                    <Box sx={{ flex: 1, overflowY: 'auto', maxHeight: 400, width: '100%' }}>
                        {payments.length === 0 ? (
                            <Box sx={{ 
                                textAlign: 'center', 
                                py: 4, 
                            ...
                            <Stack spacing={1} sx={{ width: '100%', mx: 'auto', maxWidth: isFullyPaid ? 600 : '100%' }}>
                                {payments.map((p, idx) => {
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                                <Box sx={{ 
                                                    ...
                                                    {React.cloneElement(details.icon as React.ReactElement, { fontSize: 'small' })}
                                                </Box>
                                                <Box>
                                            ...
                                            <IconButton 
                                                ...
                                                sx={{ opacity: 0.6, '&:hover': { opacity: 1, bgcolor: 'error.50' } }}
                                            >
                    ...
                    <Box sx={{ 
                        ...
                        <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
                            <Typography variant="body2" color="text.secondary">المطلوب</Typography>
                            <Typography variant="body2" fontWeight="bold">{formatNumber(grandTotal)}</Typography>
                        </Stack>
